...
struct CD3DX12_ROOT_DESCRIPTOR : public D3D12_ROOT_DESCRIPTOR
{
	...
	static void Init(_Out_ D3D12_ROOT_DESCRIPTOR& table, const UINT shaderRegister,
	                 const UINT registerSpace = 0) noexcept
	{
...
struct CD3DX12_CPU_DESCRIPTOR_HANDLE : public D3D12_CPU_DESCRIPTOR_HANDLE
{
	...
	static void InitOffsetted(_Out_ D3D12_CPU_DESCRIPTOR_HANDLE& handle, _In_ const D3D12_CPU_DESCRIPTOR_HANDLE& base,
	                          const INT offsetScaledByIncrementSize) noexcept
	{
		handle.ptr = static_cast<SIZE_T>(static_cast<INT64>(base.ptr) + static_cast<INT64>(
			offsetScaledByIncrementSize));
	}
	                          ...
	                          const INT offsetInDescriptors, const UINT descriptorIncrementSize) noexcept
	{
		handle.ptr = static_cast<SIZE_T>(static_cast<INT64>(base.ptr) + static_cast<INT64>(offsetInDescriptors) *
			static_cast<INT64>(descriptorIncrementSize));
	}
};
...
struct CD3DX12_GPU_DESCRIPTOR_HANDLE : public D3D12_GPU_DESCRIPTOR_HANDLE
{
	...
	static void InitOffsetted(_Out_ D3D12_GPU_DESCRIPTOR_HANDLE& handle, _In_ const D3D12_GPU_DESCRIPTOR_HANDLE& base,
	                          const INT offsetScaledByIncrementSize) noexcept
	{
		handle.ptr = static_cast<UINT64>(static_cast<INT64>(base.ptr) + static_cast<INT64>(
			offsetScaledByIncrementSize));
	}
...
//------------------------------------------------------------------------------------------------
constexpr UINT D3D12CalcSubresource(const UINT MipSlice, const UINT ArraySlice, const UINT PlaneSlice,
                                    const UINT MipLevels, const UINT ArraySize) noexcept
{
...
void D3D12DecomposeSubresource(const UINT Subresource, const UINT MipLevels, const UINT ArraySize, _Out_ T& MipSlice,
...
struct CD3DX12_RESOURCE_DESC : public D3D12_RESOURCE_DESC
{
	...
	UINT CalcSubresource(const UINT MipSlice, const UINT ArraySlice, const UINT PlaneSlice) const noexcept
	{
	...
	const UINT NumSlices) noexcept
{
	for (UINT z = 0; z < NumSlices; ++z)
	{
		...
		const auto pSrcSlice = static_cast<const BYTE*>(pSrc->pData) + pSrc->SlicePitch * static_cast<LONG_PTR>(z);
	...
	const UINT NumSlices) noexcept
{
	...
	{
		const auto pDestSlice = static_cast<BYTE*>(pDest->pData) + pDest->SlicePitch * z;
		const auto pSrcSlice = static_cast<const BYTE*>(pResourceData) + pSrc->Offset + pSrc->DepthPitch * static_cast<
			ULONG_PTR>(z);
		for (UINT y = 0; y < NumRows; ++y)
		{
			memcpy(pDestSlice + pDest->RowPitch * y,
...
{
	UINT64 RequiredSize = 0;
	const auto MemToAlloc = (sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT) + sizeof(UINT) + sizeof(UINT64)) *
		NumSubresources;
		...
		case D3D_ROOT_SIGNATURE_VERSION_1_1:
			{
				...
				if (pParameters)
				{
					for (UINT n = 0; n < desc_1_1.NumParameters; n++)
					{
						...
						{
							const auto pDescriptorRanges_1_0 = pParameters_1_0[n].DescriptorTable.pDescriptorRanges;
...
{
	operator UINT() const noexcept { return UINT_MAX; }
};

struct DefaultSampleDesc
{
	operator DXGI_SAMPLE_DESC() const noexcept { return DXGI_SAMPLE_DESC{1, 0}; }
};
...
private:
	D3D12_STATE_SUBOBJECT* TrackSubobject(const D3D12_STATE_SUBOBJECT_TYPE Type, const void* pDesc)
	{
